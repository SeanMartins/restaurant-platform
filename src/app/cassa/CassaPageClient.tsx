...
export default function CassaPageClient() {
  ...
  const chiudiTavolo = async (numero: number) => {
    ...
    try {
      const ordini = ordiniPerTavolo[numero]
      ...
      // Reset dello stato del tavolo a "libero": non deve bloccare la chiusura del conto se fallisce
      const tavoloId = ordini[0]?.tavoloId
      if (tavoloId) {
        try { await aggiornaTavolo(restaurantIdEffettivo, tavoloId, { stato: 'libero' }) }
        catch (err) { console.error('Impossibile aggiornare lo stato del tavolo:', err) }
      }
