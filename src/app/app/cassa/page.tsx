...
export default function CassaPage() {
  ...
  useEffect(() => {
    if (!appUser?.restaurantId) return
    const unsub = ascoltaOrdiniCassa(appUser.restaurantId, setOrdini)
    return () => unsub()
  }, [appUser])
  ...
  const chiudiTavolo = async (numero: number) => {
    ...
    try {
      // Segna tutti gli ordini come serviti
      for (const ordine of ordiniPerTavolo[numero]) {
        await updateDoc(doc(db, 'ristoranti', appUser.restaurantId, 'ordini', ordine.id), {
          stato: 'servito',
              ...
              {tavoli.map(num => (
                ...
                >
                  ...
                  <div className="text-gray-500 text-xs mt-0.5">
                    {ordiniPerTavolo[num].length} ordini
                  </div>
                </button>
              ))}
            </div>
        ...
        {tavoloSelezionato && (
          <div className="bg-gray-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                onClick={() => setTavoloSelezionato(null)}
                ...
                <div key={idx} className="flex items-center justify-between py-2 border-b border-gray-700">
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      riga.stato === 'pronto' || riga.stato === 'servito'
            ...
            <button
              onClick={() => chiudiTavolo(tavoloSelezionato)}
